...
import { login, forgotPassword, formatAuthError } from "@/services/auth";
...
export default function Login() {
  ...
  const submitForgot = async (data: ForgotForm) => {
    setForgotError("");
    try {
      await forgotPassword(data.forgotEmail.trim().toLowerCase());
      setForgotSent(true);
    } catch (err) {
          ...
          {showForgot ? (
            <div>
              <button
                type="button"
                onClick={closeForgot}
                className="flex items-center gap-1.5 text-[12px] text-white/35 hover:text-white/60 transition-colors mb-6"
              >
              ...
              {forgotSent ? (
                <div className="px-4 py-4 rounded-[10px] bg-violet-500/10 border border-violet-500/25 text-[13px] text-violet-300 leading-relaxed">
                  ...
                  <button
                    type="button"
                    onClick={closeForgot}
                    className="mt-4 w-full bg-violet-600 hover:bg-violet-500 text-white text-[13px] font-semibold py-2.5 rounded-[9px] transition-all"
                  >
                    Back to sign in
                  </button>
                </div>
              ) : (
                <form onSubmit={handleForgotSubmit(submitForgot)} noValidate>
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      autoComplete="email"
                      className="w-full bg-white/[0.04] border border-white/[0.09] rounded-[8px] px-3 py-2.5 text-[13px] text-white placeholder-white/20 outline-none transition-all focus:border-violet-500/50 focus:ring-2 focus:ring-violet-500/10"
                ...
                <div className="mb-2">
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <button
                      ...
                      className="text-[11px] text-violet-400/70 hover:text-violet-400 cursor-pointer transition-colors bg-transparent border-0 p-0"
                    >
                      Forgot password?
                    </button>
                  </div>
